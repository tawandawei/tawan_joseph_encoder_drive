...
/**
  ******************************************************************************
  * @brief          : Sampling collector
  ******************************************************************************
**/

uint8_t collect_data(Encoder_Struct *const e, float *const _Vm, uint8_t *_count_test, uint8_t _command_duty, uint16_t _sampling_num)
{
  _Vm[*_count_test] = e->w_speed;
  if(*_count_test >= _sampling_num)
  {
    *_count_test = 0;
    return 1; // test finished
  }
  *_count_test = *_count_test + 1;
  return 0; // test unfinished
}